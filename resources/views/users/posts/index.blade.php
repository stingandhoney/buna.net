    ...
    <div class="flex justify-center">
        <div class="w-10/12 bg-white p-4 rounded-md">
            <div class="p-6">
                ...
                <p>Posté: {{ $posts->count() }} {{\Illuminate\Support\Str::plural('article',$posts->count())}} et a reçu {{$user->receivedLikes()->count()}}</p>
            </div>
                            ...
                            @auth
                                @if(!$post->likedBy(auth()->user()))
                                    <form action="{{ route('posts.likes', $post) }}" method="post" class="mr-3">
                                        @csrf
                                        <button type="submit" class="text-blue-500">
                                            <svg class="h-5 w-5 fill-current text-blue-500"
